...
export function CommentCard({ comment, severityClass }: { comment: Comment; severityClass: string }) {
    ...
    <div className="rounded-lg border border-gray-800 bg-gray-900 p-4 space-y-2">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs text-gray-500 font-mono truncate">{comment.file}:{comment.line}</p>
          <p className="text-sm text-gray-200 mt-0.5">{comment.message}</p>
          {comment.ruleId && <p className="text-xs text-gray-600 mt-1 font-mono">{comment.ruleId}</p>}
        </div>
      </div>

      {outcome === null ? (
        <FeedbackButtons commentId={comment.id} onDone={(approved) => setOutcome(approved ? "approved" : "rejected")} />
      ) : outcome === "approved" ? (
