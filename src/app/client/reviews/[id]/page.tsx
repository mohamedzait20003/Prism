...
export default async function ReviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const res = await fetch(`${BASE}/api/client/reviews/${id}`, { cache: "no-store" });
  if (!res.ok) notFound();
        ...
        {review.comments.length === 0 ? (
          ...
          <div className="space-y-4">
            {review.comments.map((c) => (
              <CommentCard key={c.id} comment={c} severityClass={severityBadge[c.severity] ?? severityBadge.info} />
            ))}
          </div>
        )}
      </div>
